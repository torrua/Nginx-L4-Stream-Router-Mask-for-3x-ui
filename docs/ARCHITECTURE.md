...
> Документ содержит подробное инженерное описание архитектуры комплекса: маршрутизацию L4/L7, изоляцию процессов через сокеты в оперативной памяти, защиту от бесконечных циклов (Anti-Loop), тюнинг буферов HTTP/2 xHTTP, DNS-подсистему AdGuard Home и параметры обфускации UDP.

---

## 🌟 Архитектурные модули комплекса

Комплекс состоит из скрипта первичной защиты ОС (**`secure-vps.sh`**) и интеллектуального маршрутизатора Nginx Mainline (**`setup_mask.sh` v6.5.3**), полностью совместимого с ядром **Xray-core 24.9.27+ / 25.x / 26.x**.
...
## 🛠️ Первичная подготовка ОС (`secure-vps.sh`)

Скрипт первичной настройки выполняет:
* Включение алгоритма контроля перегрузок TCP BBR;
* Перенос порта SSH на нестандартный и авторизацию по Ed25519-ключам;
* Настройку межсетевого экрана UFW;
* Режим WAL для SQLite базы данных `x-ui.db` для предотвращения блокировок при одновременном доступе.\n